...
class NodeValue{
  public:
    ...
    NodeValue(int max_node, int min_node, int max_size){
...
class Solution{
    ...
    NodeValue largestBstHelper(Node* root){
        ...
        if(left.max_node < root->data && root->data < right.min_node)
            return NodeValue(max(root->data, right.max_node), min(root->data, left.min_node), 1 + left.max_size + right.max_size);
        
        return NodeValue(INT_MAX, INT_MIN, max(left.max_size, right.max_size));
    }
